...
def _run_gateway(
    ...
    *,
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        try:
            cron_session_key = f"cron:{job.id}:{int(time.time() * 1000)}"
            ...
            if "cron_session_key" in locals() and hasattr(session_manager, "delete_session"):
                session_manager.delete_session(cron_session_key)
    ...
    if sys.platform == "win32":
        # The Proactor loop can drop listening sockets for the gateway/WebSocket
        # server on some Windows Python builds. Selector keeps local serving stable.
        with suppress(Exception):
